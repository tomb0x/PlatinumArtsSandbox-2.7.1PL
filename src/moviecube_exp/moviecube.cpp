...
//    }
    ...
    {
    	//check events, update ...
    	loopv(Control::controls)
    	{
    		Control::controls[i]->update();
    	}
    }
...
//        if(!multiplayer(false)) entities::checktriggers();
...
//            showscores(true);
    ...
    void initclient()
    {
        ...
        players.add(player1);
		Control::Control* cp = Control::controls.add(&Control::GetPlayer());
		cp->connect(player1);

        //adds a basic camera
        //CameraManager::New("0");

//		Extentities::Init<Extentities::Extentity>();
		Extentities::Init<Extent>();
		Cutscenes::Init();
    }
...
//            if(d->type==ENT_PLAYER && ((DynamicEntity *)d)->ai)
    ...
    {
        if(i<players.length()) return players[i]; //return Camera::Camera();
        i -= players.length();
...
//        using namespace Camera;
        ///TODO: leave thirdperson 1|2 as it is
...
//    	MapToc::Get().writetoc(f);
	...
	{
        PlayerModel::RegisterAnimation(loadname, anim, num);
	}

//	void dummyarglist(char **arg, int num)
//	{
//		loopi(num)
//		{
//			conoutf("ARG[%d]: %s", i, arg[i]);
//		}
//	}
//	ICOMMAND(dummyarglist, "V", (char** arg, int* num), dummyarglist(arg, *num));
}
