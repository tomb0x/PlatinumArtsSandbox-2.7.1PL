...
bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    stream *f = opengzfile(ogzname, "rb");
    defformatstring(capt)("Sandbox Engine %s: %s - loading...", version, mname);
    ...
    {
        lilswap(&chdr.lightprecision, 3);
        if(octaversion<=20) conoutf(CON_WARN, "loading older / less efficient map format, may benefit from \"calclight\", then \"savecurrentmap\"");
