...
void setupsunlight()
{
    sunlightent.attr.setsize(0);
    sunlightent.type = ET_LIGHT;
    sunlightent.attr.add(0); //0
    sunlightent.attr.add(sunlightcolor.x*sunlightscale); //1
    sunlightent.attr.add(sunlightcolor.y*sunlightscale); //2
    sunlightent.attr.add(sunlightcolor.z*sunlightscale); //3
...
VARNR(lmshadows, lmshadows_, 0, 2, 2);
VARNR(lmaa, lmaa_, 0, 3, 3);
VARNR(lerptjoints, lerptjoints_, 0, 1, 1);
...
void show_calclight_progress()
{
    ...
    defformatstring(text1)("%.2f%% using %d textures", bar1 * 100, lightmaps.length());
...
static uint generatelumel(lightmapworker *w, const float tolerance, uint lightmask, const vector<const extentity *> &lights, const vec &target, const vec &normal, vec &sample, int x, int y)
{
    ...
    loopv(lights)
    {
        ...
        if(light.attached && light.attached->type==ET_SPOTLIGHT)
        {
            ...
            float maxatten = 1-cosf(max(1, min(90, int(light.attached->attr[0])))*RAD);
...
void patchlight(int *quality)
{
    ...
    if(calclight_canceled)
        ...
        conoutf("patched %d lightmaps using %.2f%% of %d textures (%.1f seconds)",
            total,
...
void lightreaching(const vec &target, vec &color, vec &dir, bool fast, extentity *t, float ambient)
{
    ...
    loopv(lights)
    {
        ...
        if(e.attached && e.attached->type==ET_SPOTLIGHT)
        {
            ...
            float maxatten = 1-cosf(max(1, min(90, int(e.attached->attr[0])))*RAD);
            float spotatten = 1-(1-ray.dot(spot))/maxatten;
            if(spotatten<=0) continue;
            intensity *= spotatten;
        }
        ...
        color.add(vec(e.attr[1], e.attr[2], e.attr[3]).mul(intensity/255));
