...
extentity *newentity(bool local, const vec &o, int type, int *attrs)
{
...
void entpaste()
{
    ...
    loopv(entcopybuf)
    {
        ...
        extentity *e = newentity(true, o, ET_EMPTY, c.attr.getbuf());
...
void entset(tagval *args, int num) //char *what, int *a1, int *a2, int *a3, int *a4, int *a5, int *a6, int *a7, int *a8)
{
	if(noentedit()) return;
...
void mpeditent(int i, const vec &o, int type, int *attrs, bool local)
{
    ...
    if(entities::getents().length()<=i)
    {
        ...
        extentity *e = newentity(local, o, type, attrs);
        entities::getents().add(e);
        addentity(i);
        attachentity(*e);
    }
