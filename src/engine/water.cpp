    ...
    loopi(MAXREFLECTIONS)
    {
        ...
        loopvj(ref.matsurfs)
        {
            ...
            if(light!=lastlight)
            {
                ...
                float lightrad = light && light->attr[0] ? light->attr[0] : worldsize*8.0f;
                const vec &lightcol = (light ? vec(light->attr[1], light->attr[2], light->attr[3]) : vec(ambient)).div(255.0f).mul(waterspec/100.0f);
