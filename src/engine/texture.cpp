...
static bool texturedata(ImageData &d, const char *tname, Slot::Tex *tex = NULL, bool msg = true, int *compress = NULL)
{
    ...
    if(!tname)
    {
        ...
        if(tex->name[0]=='<')
        {
            ...
            if(!file) { if(msg) conoutf(CON_ERROR, "nie mozna odczytac tekstury packages/%s", tex->name); return false; }
            file++;
        }
        else file = tex->name;
    ...
    else if(tname[0]=='<')
    {
        cmds = tname;
        file = strrchr(tname, '>');
        if(!file) { if(msg) conoutf(CON_ERROR, "nie mozna odczytac tekstury %s", tname); return NULL; }
        file++;
    }
    ...
    if(flen >= 4 && (!strcasecmp(file + flen - 4, ".dds") || dds))
    {
        ...
        if(!dds || dbgdds) { if(msg) conoutf(CON_ERROR, "nie mozna odczytac tekstury %s", dfile); return false; }
    }

    SDL_Surface *s = loadsurface(file);
    if(!s) { if(msg) conoutf(CON_ERROR, "nie mozna odczytac tekstury %s", file); return false; }
    ...
    while(cmds)
    {
        ...
        else if(!strncmp(cmd, "scale", len) || !strncmp(cmd, "thumbnail", len))
        {
...
void autograss(char *name)
{
    ...
    if(!strstr(name, "<ffskip>"))
        cmd = "<ffskip>";
    if(!strstr(name, "<premul>"))
        cmd = cmd ? "<ffskip><premul>" : "<premul>";

    s.autograss = name[0] ? newstring(
        makerelpath(
            strstr(name, "packages") ? NULL : "packages",
            name, NULL, cmd
        )
    ) : NULL;
}
...
Texture *cubemapload(const char *name, bool mipit, bool msg, bool transient)
{
    ...
    if(!strchr(pname, '*'))
    {
        ...
        if(!t && msg) conoutf(CON_ERROR, "nie mozna odczytac envmap %s", name);
    }
