...
{
	return bonushregen + deltahregen + getattr(STAT_ENDURANCE) * 0.0025 + getattr(STAT_STRENGTH) * 0.00125;
}
...
{
	float mul = min<float>(1, max<float>(2 - parent->getweight() / getmaxcarry(), 0.01));
